...
class ViewMoviesTest extends TestCase
{
    ...
    {
        Http::fake([
            'https://api.themoviedb.org/3/movie/*?language=fr-FR&append_to_response=credits,videos,images&include_image_language=null' => $this->fakeSingleMovie(),
        ]);

        $response = $this->get(route('movies.show', 12345));

        $response->assertSuccessful();
        $response->assertSee('Acteurs');
    ...
    {
        Http::fake([
            'https://api.themoviedb.org/3/search/movie?language=fr-FR&query=jumanji' => $this->fakeSearchMovies(),
        ]);
